...
/*
 * rio_readnb - Safly read n bytes (buffered)
 */
ssize_t
...
/* 
 * sio_read_line - Safly read a text line (buffered)
 */
ssize_t
...
/*
 * rio_writen - Safly write n bytes (unbuffered)
 */
ssize_t
...
sio_read(Sio *sio, char *usrbuf, size_t n)
{
    ...
    while (sio->sio_cnt <= 0) {  /* Refill if buf is empty */
    	sio->sio_cnt = read(sio->sio_fd, sio->sio_buf, sizeof(sio->sio_buf));
    	if (sio->sio_cnt < 0) {
    ...
    /* Copy min(n, rp->sio_cnt) bytes from internal buf to user buf */
    cnt = n;          
    if (sio->sio_cnt < n)   
	    cnt = sio->sio_cnt;
